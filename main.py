...
def add_invoice():
    ...
    if request.method == 'POST':
        try:
            ...
            # Get Village_Id from village name
            ...
            if not village_result:
                return json_response({"status": "error", "message": "Invalid Village Name"}, 400)
            ...
            cursor.execute("SET @invoice_id = 0")
            ...
            args = [
                ...
                subcontractor_id, '@invoice_id'
            ]

            cursor.callproc('SaveInvoice', args)

            # Fetch the OUT param
            cursor.execute("SELECT @invoice_id")
            invoice_id = cursor.fetchone()

            # --- Commit after procedure ---
            connection.commit()
            ...
            for hold_type, hold_amount in zip(hold_types, hold_amounts):
                ...
                if not hold_type_result:
                    return json_response({"status": "error", "message": f"Invalid Hold Type: {hold_type}"}, 400)
            ...
            return json_response(ResponseHandler.add_success("invoice"), 201)

        except mysql.connector.Error as e:
            connection.rollback()
            return json_response({"status": "error", "message": f"Failed to add invoice: {str(e)}"}, 500)
        finally:
            cursor.close()
            connection.close()
    ...
    return render_template('add_invoice.html', invoices=invoices)
...
def download_report(contractor_id):
    ...
    try:
        ...
        # Fetch invoices and GST releases
        cursor.execute("""
            SELECT DISTINCT i.Invoice_Id, i.PMC_No, v.Village_Name, i.Work_Type, i.Invoice_Details,
                   i.Invoice_Date, i.Invoice_No, i.Basic_Amount, i.Debit_Amount,
                   i.After_Debit_Amount, i.GST_Amount, i.Amount, i.TDS_Amount, i.SD_Amount,
        ...
        # Collect PMC numbers
        pmc_numbers = tuple(set(inv['PMC_No'] for inv in invoices if inv['PMC_No']))

        # Fetch payments and organize
        payments_map, extra_payments_map = {}, {}
        if pmc_numbers:
            query = f"""
                SELECT pmc_no, invoice_no, Payment_Amount, TDS_Payment_Amount, Total_amount, UTR
                FROM payment
            ...
            payments = cursor.fetchall()
            for pay in payments:
                key = (pay['pmc_no'], pay['invoice_no']) if pay['invoice_no'] else pay['pmc_no']
                if pay['invoice_no']:
                    payments_map.setdefault(key, []).append(pay)
                else:
                    extra_payments_map.setdefault(key, []).append(pay)
        ...
        # Contractor Info
        for label, value in contInfo.items():
            sheet.append([label.replace('_', ' ').title(), value])
        sheet.append([])
        ...
        base_headers = ["PMC No", "Village", "Work Type", "Invoice Details", "Invoice Date", "Invoice No",
                        "Basic Amount", "Debit", "After Debit Amount", "GST (18%)", "Amount", "TDS (1%)",
                        "SD (5%)", "On Commission", "Hydro Testing", "GST SD Amount"]
        hold_headers = list(hold_type_map.values())
        payment_headers = ["Final Amount", "Payment Amount", "TDS Payment", "Total Paid", "UTR"]
        sheet.append(base_headers + hold_headers + payment_headers)
        ...
        for inv in invoices:
            pmc_no, invoice_no = inv["PMC_No"], inv["Invoice_No"]
            key = (pmc_no, invoice_no)
            payments = payments_map.get(key, [])
            ...
            row += [
                inv["Final_Amount"],
                first_payment.get("Payment_Amount", ""),
                first_payment.get("TDS_Payment_Amount", ""),
                first_payment.get("Total_amount", ""),
                first_payment.get("UTR", "")
            ]

            payment_id = f"{pmc_no}-{invoice_no}-{first_payment.get('Payment_Amount', '')}-{first_payment.get('UTR', '')}"
            if payment_id:
                processed_payments.add(payment_id)
            ...
            for pay in payments[1:]:
                payment_id = f"{pmc_no}-{invoice_no}-{pay['Payment_Amount']}-{pay.get('UTR', '')}"
                if payment_id not in processed_payments:
                    extra_row = [pmc_no, "", "", "", "", invoice_no] + [""] * (len(base_headers) - 6)
                    extra_row += [""] * len(hold_headers)
                    extra_row += [inv["Final_Amount"], pay["Payment_Amount"], pay["TDS_Payment_Amount"], pay["Total_amount"], pay["UTR"]]
                    sheet.append(extra_row)
                    processed_payments.add(payment_id)

        # Extra payments (null invoice_no)
        for pmc_no, payments in extra_payments_map.items():
            for pay in payments:
                payment_id = f"{pmc_no}-null-{pay['Payment_Amount']}-{pay.get('UTR', '')}"
                if payment_id not in processed_payments:
                    row = [pmc_no, "", "", "", "", ""] + [""] * (len(base_headers) - 6)
                    row += [""] * len(hold_headers)
                    row += ["", pay["Payment_Amount"], pay["TDS_Payment_Amount"], pay["Total_amount"], pay["UTR"]]
                    sheet.append(row)
                    processed_payments.add(payment_id)

        # Totals
        totals = {
            "Basic": 0, "TDS": 0, "SD": 0, "Commission": 0, "Hold": 0, "Final": 0, "TotalPaid": 0
        }

        for row in sheet.iter_rows(min_row=2, values_only=True):
            try:
                totals["Basic"] += float(row[6] or 0)
                totals["TDS"] += float(row[11] or 0)
                totals["SD"] += float(row[12] or 0)
                totals["Commission"] += float(row[13] or 0)
                totals["Final"] += float(row[-5] or 0)
                totals["TotalPaid"] += float(row[-2] or 0)
                totals["Hold"] += sum(float(row[16 + i] or 0) for i in range(len(hold_headers)))
            except:
                continue

        sheet.append([])
        total_row = ["TOTAL", "", "", "", "", "", totals["Basic"], "", "", "", "", totals["TDS"], totals["SD"],
                     totals["Commission"], "", ""]
        total_row += [totals["Hold"]] + [""] * (len(hold_headers) - 1)
        total_row += [totals["Final"], "", "", totals["TotalPaid"], ""]
